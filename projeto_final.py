...
def inserir_usuario():
    ...
    if cpf and nome and idade and numero and email:
    #    try:
            conn  =  conectar()
            c = conn.cursor()
            c.execute('INSERT INTO usuarios (id, nome, idade, numero, email) VALUES (?,?,?,?,?)', (cpf, nome, idade, numero, email))
            conn.commit()
            conn.close()
...
def editar():
    ...
    if selecao:
        ...
        novo_numero =  entry_numero.get()
        novo_email = entry_email.get()
        
       
        if  novo_nome and nova_idade and novo_numero and novo_email:
            try:
                    conn  =  conectar()
                    c = conn.cursor()
                    c.execute('UPDATE usuarios SET nome = ?,  email = ? WHERE id = ?', (novo_nome, nova_idade, novo_numero, novo_email,user_id))
                    conn.commit()
                    conn.close()
                    messagebox.showinfo('Dados','DADOS INSERIDOS COM SUCESSO!')
                    mostra_usuario()
...
titulo = ttk.Label(main_frame, text='Armazenamento de Cadastros', font=('Segoe UI', 10, 'bold'))
...
#Número
ttk.Label(input_frame, text='NÚMERO').grid(row=3, column=0, padx=(0,10), pady=5, sticky='e')

entry_numero = ttk.Entry(input_frame, width=30)
entry_numero.grid(row=3, column=1, padx=(0,20), pady=5, sticky='w')



# E-MAIL
ttk.Label(input_frame, text='E-MAIL').grid(row=4, column=0, padx=(0,10), pady=5, sticky='e')

entry_email = ttk.Entry(input_frame, width=30)
entry_email.grid(row=4, column=1, padx=(0,20), pady=5, sticky='w')
...
btn_salvar = ttk.Button(btn_frame, text='ARMAZENAR', command=inserir_usuario)
...
# criação da TreeView
columns = ('ID', 'NOME', 'IDADE','NÚMERO','E-MAIL')
